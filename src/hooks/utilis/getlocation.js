  ...
  if (matchedUrl) {
    ...
    if (!formateAddresing) {
      const response = await fetch(
        `http://maps.googleapis.com/maps/api/geocode/json?address=${value}&key=AIzaSyBRchIzUTBZskwvoli9S0YxLdmklTcOicU`
      );
      ...
      if (data.error_message) {
        ...
        if (
          Boolean(object.administrative_area_level_3) ||
          Boolean(object.establishment) ||
          Boolean(object.locality) ||
          Boolean(object.sublocality) ||
          Boolean(object.administrative_area_level_2)
        ) {
          ...
          if (
            Boolean(object.administrative_area_level_3) &&
            Boolean(object.locality) &&
            Boolean(object.administrative_area_level_1) &&
            Boolean(object.locality)
          ) {
            ci = object.locality.replace(/\s/g, "-");
            citycode = short.locality;
        ...
        if (
          Boolean(object.route) ||
          Boolean(object.sublocality_level_2) ||
          Boolean(object.neighborhood) ||
          Boolean(object.establishment)
        ) {
        ...
        const createurl = Boolean(route)
          ? `http://www.weedx.io/weed-${weburl}/in/${modifystr(
              country
          ...
          : Boolean(country) &&
            `http://www.weedx.io/weed-${weburl}/in/${modifystr(country)}`;
