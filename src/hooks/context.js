...
const Context = (props) => {
  ...
  useEffect(() => {
    const cookies = new Cookies();
    const userToken =
      cookies.get("User_Token_access") || getTokenFromLocalStorage();
    ...
    const initializeCart = async (token) => {
      ...
      try {
        ...
        const cartSubTotal = cartProducts.reduce((total, product) => {
          const productPrice = product.DiscountedAmount
        ...
        const calculatedDeliveryPrice =
          cartSubTotal > minimumOrderPrice ? 0 : deliveryPrice;

        dispatch({ type: "AllProduct", AllProduct: cartProducts });
        dispatch({
          type: "Cart_subTotal",
          Cart_subTotal: cartSubTotal + calculatedDeliveryPrice,
        });
        dispatch({
          type: "DeliveryPrice",
