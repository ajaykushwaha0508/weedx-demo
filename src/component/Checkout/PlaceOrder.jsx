        ...
        <div className="col-12 col-lg-8 col-sm-10">
          <div className={newclass.OrderDonePage}>
            <div className={newclass.order_conform_card}>
              <span>
                <FaRegPaperPlane
                  className={newclass.order_conform_card_icons}
                />
              </span>
              <h3 className={newclass.card_title}>{`Thank You!`}</h3>
              <p
                className={newclass.card_message}
              >{`You'll receive a confirmation email soon`}</p>
              <Link
                href={
                  location.pathname === "/menu-integration/order-placed"
                    ? `/menu-integration/MyOrderProductDetail/${Order[0]?.OrderId}`
                    : `/MyOrderProductDetail/${Order[0]?.OrderId}`
                }
                className="mt-4"
              >
                <button
                  className={newclass.trackorderbtn}
                >{`Track Order`}</button>
              </Link>
            </div>
            <div className="d-flex justify-content-between align-items-center">
              <p className={`${newclass.card_message} m-0`}>
                {" "}
                <b>{`Order ID`}</b> : #{location.query.OrderId}{" "}
              </p>
              <p className={`${newclass.card_message} m-0`}>
                {" "}
                <b>{`Order Date`}</b> : {formatted}{" "}
              </p>
            </div>
              ...
              <div className="col-lg-8">
                {Order[0]?.Product?.map((item, index) => {
                  ...
                  return (
                    <div
                      key={index}
                      className={newclass.place_order_product_cart}
                    >
                      ...
                      <div className={newclass.place_order_product_cart_Text}>
                        <h4 className={newclass.productname}>
                          {item.ProductName}
                        </h4>
                        <p>
                ...
                <div className="  ml-auto p-sm-3 p-2 border">
                  ...
                  <div className="d-flex justify-content-between align-items-center">
                    <div style={{ display: "grid" }}>
                      <span>{`Total`}</span>
                      {Boolean(pricess.discount) && (
                        <span>{`Discount Amount`}</span>
                      )}
                    </div>
                    <div style={{ display: "grid" }}>
                      {Boolean(pricess.discount) ? (
                        <del>${pricess.Subtotal}</del>
                      ) : (
                        <span>${pricess.Subtotal}</span>
                      )}
                      {Boolean(pricess.discount) && (
                        <span>${pricess.Subtotal - pricess.discount}</span>
                      )}
                    </div>
                  </div>
                </div>
            ...
            <div className={`row border ${newclass.place_order_address}`}>
              <div className="col-md-4 p-sm-4 p-2">
                <h4>{`Seller Info`}</h4>
                <p>
                  {`Name`} : {Order[0]?.StoreName}
                </p>
                <p>
                  {`Address`} : {Order[0]?.StoreAddress}
                </p>
              </div>
              <div className="col-md-4 p-sm-4 p-2">
                <h4>
                  {location.query.orterbtn === "pickup_btn"
                    ? "Curbside Pickup"
                    : "Delivery "}{" "}
                  {`Address`}
                </h4>
