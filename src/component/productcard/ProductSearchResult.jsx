...
const ProductSearchResult = ({
  ...
  async function AddToCart(Event, counter, SelectWeight, handleClose) {
    ...
    const PriceArrry = _.find(
      AddData[0].Price,
      (Price) => Price.id === SelectWeight
    );
    const FinalSelection =
      PriceArrry === undefined ? Event.Prices[0].Price[0] : PriceArrry;
    const FinalPriceId =
      PriceArrry === undefined ? Event.Prices[0].Price[0].id : PriceArrry.id;
      ...
      if (AddTOCard.length !== 0) {
        if (
          ...
          const t = AddTOCard.filter((data) => {
            return (
              data.Product_id === Event.id && data.Price.id === FinalPriceId
            );
          });
          if (t.length > 0) {
            SetAddToCard(
  ...
  function discountshoer(CategoryCoupoun, ProductCoupoun) {
    ...
    if (newarr[0].DiscountType === "Amount off Order") {
      let showdata = "";
      newarr[0].PercentageAmount === null
        ? (showdata = "Flat " + newarr[0].ValueAmount + " Off")
        : (showdata = `Save ${newarr[0].PercentageAmount} %`);
      return showdata;
      ...
      <div className="">
        ...
        title === "Explore Nearby Products" ? (
          <div className={"product_card_wrapper"}>
            <Swiper
              className={`mySwiper similerproduxt`}
              spaceBetween={50}
              slidesPerView={6}
              navigation={true}
              modules={[Navigation]}
                  ...
                  return (
                    <SwiperSlide key={index}>
                      <div className={"productSearch_result_container"}>
                        {parseInt(items.Prices[0].Price[0].Price) >
                          parseInt(items.Prices[0].Price[0].SalePrice) && (
                        ...
                        <div className={"productSearchResultImage_container"}>
                          ...
                          <Link
                            href={`/${link}/${modifystr(
                              items.category_name
                            )}/${modifystr(items.SubcategoryName)}/${modifystr(
                              items.Product_Name
                            )}/${items.id}`}
                            state={{
                        ...
                        <div className={"product_search_result_content_div"}>
                          ...
                          >
                            <p
                              className={`${"productSearchResultParagraph"} text-truncate`}
                            >
                              {items.Product_Name}
                            </p>
                            <p
                              className={`${"product_search_result_sub_heading"} text-truncate`}
                            >
                              by {items.StoreName}
                            </p>
                            <div className={"product_category_list"}>
                              <span className={"product_search_result_span1"}>
                                15
                                {items.lab_Result !== "Magnesium"
                                  ? "%"
                                  : "Mg."}{" "}
                                {`THC | 0.2`}
                                {items.lab_Result !== "Magnesium"
                            ...
                            <p
                              className={`productSearch text-truncate text-dark`}
                            >
                              <span className={"productSearchPrice"}>
                                $
                                {parseInt(items.Prices[0]?.Price[0]?.SalePrice)}{" "}
                                {parseInt(items.Prices[0].Price[0].Price) >
                                  parseInt(
                                    items.Prices[0].Price[0].SalePrice
                                  ) && (
                              ...
                              {`per`}{" "}
                              {items.Prices[0].Price[0].Weight
                                ? items.Prices[0].Price[0].Weight
                                : `${items.Prices[0].Price[0].Unit} Unit`}
                            </p>
                            <div>
                              {" "}
                              {items?.CategoryCoupoun?.length !== 0 ||
                                (items?.ProductCoupoun?.length !== 0 && (
                                  <div className="discountinfo">
                                    <span className={"carddiscountoffer"}>
                                      {discountshoer(
                                        items.CategoryCoupoun,
                                        items.ProductCoupoun
                                      )}{" "}
                                    </span>
                                    {`  and more Offers`}{" "}
                                  </div>
                          ...
                          >
                            {items?.Prices[0].Price.length > 1 ? (
                              <ProductIncDecQuantity
                                ...
                                AddToCart={AddToCart}
                              />
                            ) : items?.Prices[0].Price[0].Stock ===
                              "IN Stock" ? (
                              <LoadingButton
                                loading={adding === items.id}
                                loadingIndicator={
          ...
          <div className={"product_card_wrapper"}>
            {RelatedProductResult?.map((items, index) => {
              if (items.id !== currentProductID) {
                return (
                  <div
                    className={`productSearch_result_container productSearch_result_containermainapge`}
                    key={index}
                  >
                    {parseInt(items.Prices[0].Price[0].Price) >
                      parseInt(items.Prices[0].Price[0].SalePrice) && (
                    ...
                    <div className={"product_search_result_content_div"}>
                      ...
                      {state.Embedded_Store.StoreID === "" && (
                        <Link
                          href={`/${
                            items.Store_Type === "dispensary"
                              ? "weed-dispensaries"
                              : "weed-deliveries"
                          }/${modifystr(items.StoreName)}/${items.Store_id}`}
                        >
                          <p
                            className={`product_search_result_sub_heading text-truncate`}
                          >
                      ...
                      <div className={"product_category_list"}>
                        <span className={"product_search_result_span1"}>
                          15{items.lab_Result !== "Magnesium" ? "%" : "Mg."} THC
                          | 0.2{items.lab_Result !== "Magnesium" ? "%" : "Mg."}{" "}
                          {`CBD`}
                        </span>
                        <div className={"d-flex gap-1"}>
                          {new Array(items.rating)
                          ...
                          <span className={"product_search_result_sub_heading"}>
                            ({items.TotalRating})
                          </span>
                        </div>
                      </div>
