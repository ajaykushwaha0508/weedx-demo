...
const NewProductDetailsCards = ({
  ...
  const Addtocard = async (Event) => {
    if (token_data) {
      ...
      const PriceArrry = h.find(
        (data) => data.id === parseInt(AddData[0]?.Item_id) && data
      );
      let PriceIndex =
      ...
      SetNewData({
        ...
        CoupounField:
          DiscountedValue.DiscountType === "" ? null : DiscountedValue,
        PromoCodeid: DiscountedValue.id,
        CustomerGets:
          DiscountedValue.DiscountType === "Buy X get Y"
            ? DiscountedValue.CustomerGets
            : null,
            ...
            SetAddToCard(
              AddTOCard.map((Cart) => {
                if (
                  ...
                  return {
                    ...Cart,
                    Cart_Quantity: Cart.Cart_Quantity + quentity,
                  };
                }
                return Cart;
              })
    ...
    Product?.Prices[0]?.Price?.forEach((item, index) => {
      if (item.SalePrice === val && item.Quantity - 1 >= quentity) {
        ...
        Swal.fire({
          ...
          footer: `The maximum available quantity for this item is ${
            item.Stock === "Out of Stock" ? "0" : quentity
          }.`,
          timer: 4000,
          ...
          <div className={newclases.newProductDetailsContainer}>
            <div className={newclases.newProductDetailsCardLeftCol}>
              {Product?.images?.length > 1 && (
                <div className={newclases.newProductDetailsCardimagelist}>
                  {Product?.images?.map((items, index) => {
                    return (
                      <div
                        key={index}
                        className={`${
                          newclases.NewProductDetails_image_container
                        } ${displaypic === items.image && newclases.active}`}
                      >
                        <Image
                          priority
                          width={150}
              ...
              <div className={newclases.newProductDetailsUpperimage_container}>
                <Image
                  className={newclases.newProductDetails_upper_image}
                  width={100}
                  priority
                  height={100}
                  src={
                    Boolean(displaypic) ? displaypic : Product?.images[0]?.image
                  }
                  alt={Product?.Product_Name}
                  onError={(e) => (e.target.src = "/blankImage.jpg")}
                  title={Product?.Product_Name}
            ...
            <div
              className={newclases.newProductdetails_rightSideContent_container}
            >
              <h1 className={newclases.newProductDetails_heading}>
                {Product?.Product_Name}
              </h1>
              {state.Embedded_Store.StoreID === "" && (
                <Link
                  href={`/${link}/${modifystr(Product?.StoreName)}/${
                    Product?.Store_id
                  }`}
                >
                  <h3 className={newclases.newProductDetails_subHeadingss}>
              ...
              <div className={newclases.newProductDetailsButon}>
                {Product.THC !== 0 && (
                  <button className={newclases.newProductdetailsButtonss}>
                    {Product.THC}% THC
                  </button>
                )}
                {Product.CBD !== 0 && (
              ...
              <p className="py-2 d-flex align-items-center gap-1">
                <Rating
                  name="read-only"
                  className={`${classes.homePageStarIconscolor}`}
                  value={
                    Product.rating === null ? 0 : parseInt(Product?.rating)
                  }
                  size="small"
                  readOnly
                />
                <span>
                  {Product.rating === null ? 0 : Product.rating + ".0"}{" "}
              ...
              <div className={newclases.productDetailsCardQuestity}>
                <span className={newclases.newProduct_Weight}>
                  {`Quantity :`}{" "}
                </span>

                <div className={newclases.qty_selector}>
                  <span
                    className={newclases.qty_btn}
                    onClick={() => {
                      if (quentity > 1) {
                        setquentity(quentity - 1);
                      }
                    ...
                    <div className={newclases.DisplayDiscount}>
                      <span>
                        {parseInt(dynamicWeight) !== 0
                          ? parseInt(dynamicWeight * quentity) -
                            (Boolean(DiscountedValue?.Percentage)
                              ? (dynamicWeight *
                                  quentity *
                                  parseInt(DiscountedValue?.Percentage)) /
                                100
                              : parseInt(DiscountedValue.Amount))
                          : Product?.Prices?.map((data) => {
                              return (
                                data.Price[0].SalePrice * quentity -
                                (Boolean(DiscountedValue?.Percentage)
                                  ? parseInt(
                                      (data.Price[0].SalePrice *
                                        quentity *
                                        parseInt(DiscountedValue?.Percentage)) /
                                        100
                                    )
                      ...
                      <strike>
                        {parseInt(dynamicWeight) !== 0
                          ? dynamicWeight
                          : Product?.Prices?.map(
                              (data) => data.Price[0].SalePrice * quentity
                            )}
                      </strike>
                    </div>
                  ) : parseInt(dynamicWeight) !== 0 ? (
                    (dynamicWeight * quentity).toFixed(1)
                  ) : (
                    Product?.Prices?.map((data) =>
                      (data.Price[0].SalePrice * quentity).toFixed(1)
                    )
                  )}
                </span>
                <span className={newclases.newProduct_Gms}>
                  / {quentity} {`piece`}
                </span>
                {DiscountedValue?.Reflect && (
                  <span
                    className="mx-3 newProduct_Gms"
                    style={{ color: "#31B665" }}
                  >
                    Offer Applied
                  </span>
              ...
              <div className={newclases.productDetailsCardAddtocart}>
                {Product?.Prices?.map((data, index) => {
                  if (dynamicWeight === 0) {
                    ...
                    return data.Price.map((arry, index) => {
              ...
              <div className="pt-4">
                <Accordion className={classes.productdescription}>
                  ...
                  <AccordionDetails>
                    <div
                      className="newProductAboutUs_description"
            ...
            <div className="position-absolute w-auto top-0 p-2  end-0">
              ...
              <span className="shareiconcontainer"></span>
            </div>
          </div>
