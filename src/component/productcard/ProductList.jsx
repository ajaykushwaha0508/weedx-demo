...
const ProductList = ({ arr, link = "products" }) => {
  ...
  const pagechanges = (event, value) => {
    ...
    let assa = showdata.slice(
      value * productperpage - productperpage,
        ...
        !state?.Loading ? (
          <React.Fragment>
            <div className="row h-auto mb-2  mx-2">
                      ...
                      <div className={"prod_cat_cont"}>
                        <div className={`col-12 ${"prod_cat_img"}`}>
                          <Link
                            href={`/${link}/${modifystr(
                              ele.category_name
                            )}/${modifystr(ele.SubcategoryName)}/${modifystr(
                              ele.Product_Name
                            )}/${ele.id}`}
                            state={{
                              ...
                              onClick={() => {
                                Navigate.push(
                                  `/${link}/${modifystr(
                                    ...
                                    ele.SubcategoryName
                                  )}/${modifystr(ele.Product_Name)}/${ele.id}`
                                );
                              }}
                              width={100}
                              priority
                          ...
                          <div className={"prod_img_btn"}>
                            {ele.THC !== 0 && (
                              <button
                                className={`${"cat_prod_inner_btn"} ${"btn2"}`}
                              >
                                THC {ele.THC}{" "}
                                {ele.lab_Result !== "Magnesium" ? "%" : "Mg."}
                              </button>
                            )}
                            {ele.CBD !== 0 && (
                              <button
                            ...
                            {ele.strain !== "None" && (
                              <button
                                className={`${"cat_prod_inner_btn"} ${"btn1"}`}
                              >
                                {ele.strain}
                              </button>
                            )}
                      ...
                      >
                        <div className={"product_cat_allProduct"}>
                          <div className="col-12 ">
                            <h3
                              className={`${"productListHeadings"} ellipsis`}
                              title={ele.Product_Name}
                            >
                              {" "}
                              {ele.Product_Name}{" "}
                          ...
                          <div className="text-dark">
                            <p
                              className={`${"productSearch"} text-truncate mb-0`}
                            >
                              <span className="productSearchPrice">
                                ${parseInt(ele.Prices[0]?.Price[0]?.SalePrice)}{" "}
                                {parseInt(ele.Prices[0].Price[0].Price) >
                                  parseInt(
                                    ele.Prices[0].Price[0].SalePrice
                                  ) && (
                              ...
                              per{" "}
                              {ele.Prices[0].Price[0].Weight
                                ? ele.Prices[0].Price[0].Weight
                                : `${ele.Prices[0].Price[0].Unit} Unit`}
                            </p>
                          </div>
                        </div>
                      </Link>
              ...
              {showdata.length > productperpage &&
                location.pathname.includes("/menu-integration") && (
                  <div className="d-flex justify-content-center">
                    <Pagination
                      count={
                        showdata.length % productperpage === 0
                          ? parseInt(showdata.length / productperpage)
                          : parseInt(showdata.length / productperpage) + 1
                      }
                      page={page}
