    ...
    axios
      ...
      .then((response) => {
        if (response.data !== "Not Authorised") {
          ...
          cookies.set("User_Token_access", response?.data?.tokens?.access, {
            expires: date,
          });
          ...
          if (
            location.state === null ||
            location.state.location.pathname === "/cart"
          ) {
            Navigate(-1);
          } else {
            Navigate("/");
          ...
          Swal.fire({
            ...
            footer:
              "Invalid email or password. Please check and try again. If not registered, use the correct email or sign up for a new account.",
            customClass: {
    ...
    <React.Fragment className="my-3">
      ...
      <div className="login_signup_reset_container ">
          ...
          <div className="row mt-3">
            <div className=" col-lg-12 ">
              <Box
                className={
                  !loading
                    ? `${classes.loginBtnTextAndBackground}`
                    : `${classes.loginBtnTextAndBackgroundAfter}`
                }
              >
                <LoadingButton
                  variant="outlined"
        ...
        <div className="w-100 d-flex mt-4 center align-items-center">
          <div className="login_horizontalLine "></div>{" "}
          <span className="px-2 login_OR">OR</span>{" "}
          <div className="login_horizontalLine "></div>
        </div>
        <div className="row mt-3">
          <div className="col-lg-12 ">
        ...
        <div className="w-100 text-center my-2 ">
          <h2 className="login_bottom">
            {`New To weedx ?`}{" "}
            <span>
              <link href={"/signup"} className="signInfo">{`Sign Up`}</link>
            </span>
          </h2>
          <Link className="signInfo" href={"/signup"}>
            <span>{`Create your Weedx Account`}</span>
          </Link>
        </div>
