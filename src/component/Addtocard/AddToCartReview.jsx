                ...
                {   
                                ...
                                return (
                                    <div className="row py-3 px-0 border-top border-bottom justify-content-center align-items-center" key={index}>
                                        <div className='row align-items-center col-5'>
                                            ...
                                            <div className={newclasess.Add_prod_content_cont}>
                                                <h5 className={`${newclasess.add_prod_cart_p_title} ${ele?.Price?.Stock !== "IN Stock" && 'text-danger'}`} onClick={() => { navigate(ele) }}>{ele.ProductName + "(" + wrigh + ")"}</h5>
                                                <h4 className={`${newclasess.add_prod_cart_p} ${ele?.Price?.Stock !== "IN Stock" && 'text-danger'}`}>{ele.StoreName}</h4>
                                                {ele?.Price?.Stock !== "IN Stock" && <h3 className={`${newclasess.add_prod_cart_p} text-danger`}>{`This Product is Out of Stock Now Please remove it from the cart`}</h3>}
                                            </div>
                                        </div>
                                        <div className="col-2 text-center">
                                            <span className={newclasess.add_prod_span_amount}>${parseInt(ele.Price.SalePrice)}</span>
                                        </div>
                                        <div className="col-2 text-center">
                                            <div className={newclasess.AddToCartReviewBtn}>
                                                <div className={newclasess.addToCart_btn}>
                                                ...
                                                <div className={newclasess.AddToCartCount} style={{ width: "20px" }}>
                                                    <p className={newclasess.addToCartCountNumber}>{ele.Cart_Quantity}</p>
                                                </div>
                                                <div className={newclasess.addToCart_btn}>
                                        ...
                                        <div className="col-2 text-center">
                                            <span className={newclasess.add_prod_span_amount} value={ele.Price.SalePrice * ele.Cart_Quantity} >${parseInt(ele.Price.SalePrice * ele.Cart_Quantity)}</span>
                                        </div>
                    ...
                    <div >
                        {AfterDiscount?.map((ele, index) => {
                            ...
                            return (
                                <div className={newclasess.addtoproduct_card} key={index}>
                                    <div className={newclasess.mb_addtoproduct_card_img}>
                                        <Link href={`/products/${modifystr(ele.category)}/${modifystr(ele.SubcategoryName)}/${modifystr(ele.ProductName)}/${ele.Product_id}`}>
                                            <Image   width={100}  height={100} src={`${ele.Image}`} alt={ele.ProductName}  title={ele.ProductName}
                                                priority  onError={(e) => (e.target.src = '/blankImage.jpg')} />
                                        </Link>
                                    </div>
                                    <div className={newclasess.mb_addtoproduct_card_content}>
                                        <div className='d-flex align-items-start'>
                                            <h5 className={`${newclasess.add_prod_cart_p_title} col-10 ${ele?.Price?.Stock !== "IN Stock" && 'text-danger'}`} onClick={() => { Navigate.push(ele) }}>{ele.ProductName + "(" + wrigh + ")"}</h5>
                                            <span className='col-2 d-flex justify-content-center align-items-center'><button loading={LoadingDelete} className="center border-0 bg-transparent" onClick={(() => { DeleteItem(ele.Product_id, ele.id) })}> <RiDeleteBin6Line className={newclasess.delete_icons_add} color='gray' size={22} /></button></span>
                                        </div>
                                        <div className='d-flex justify-content-between align-items-end'>
                                            <div className={newclasess.AddToCartReviewBtn}>
                                                <div className={newclasess.addToCart_btn}>
                                                    {(Loadingmines || ele.Cart_Quantity) > 1 ? <LoadingButton loading={Loadingmines} style={{ width: "15px" }} onClick={() => { decreaseQuantity(ele.id, ele) }} >  <GrFormSubtract color='gray' /></LoadingButton> : <span><LoadingButton loading={LoadingDelete} className="center" style={{ width: "15px" }} onClick={(() => { DeleteItem(ele.Product_id, ele.id) })}> <RiDeleteBin6Line className={newclasess.delete_icons_add} color='gray' /></LoadingButton></span>}
                                                </div>
                                                <div className={`${newclasess.AddToCartCount} text-center`} >
                                                    <p className={newclasess.addToCartCountNumber}>{ele.Cart_Quantity}</p>

                                                </div>
                                                <div className={newclasess.addToCart_btn}>
                                                    <LoadingButton loading={LoadingPlue} className="center" style={{ width: "15px" }} onClick={() => { Quantity(ele.id, ele.Cart_Quantity, ele) }} sx={{
                                                        minWidth: '30px', }} ><AiOutlinePlus color='gray' /></LoadingButton>
                                                </div>
                                            </div>
                                            <span className={newclasess.add_prod_span_amount} value={ele.Price.SalePrice * ele.Cart_Quantity} >${parseInt(ele.Price.SalePrice * ele.Cart_Quantity)}</span>
                                        </div>
