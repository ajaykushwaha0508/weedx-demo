...
const AddToCartReview = () => {
      ...
      if (result.isConfirmed) {
        ...
        if (state?.login) {
          ...
          try {
            ...
            await Axios.delete(
              `https://api.cannabaze.com/UserPanel/DeleteAddtoCart/${id}`,
              config
            );
  ...
  async function Quantity(Id, Cart, Event) {
    if (Event?.Price?.Quantity > Event.Cart_Quantity) {
      if (state?.login || token_data) {
        ...
        await Axios.post(
          `https://api.cannabaze.com/UserPanel/Update-AddtoCart/${Id}`,
          Arry,
          config,
        ...
        var s = obj?.map((arr) => {
          if (
            arr.Product_id === Event.Product_id &&
            arr.Price.id === Event.Price.id
          ) {
  ...
  function navigate(e) {
    Navigate.push(
      `/products/${modifystr(e.category)}/${modifystr(
        e.SubcategoryName
      )}/${modifystr(e.ProductName)}/${e.Product_id}`
    );
  }
  useEffect(() => {
  ...
  return (
    <div className={"AddProductCartContainerinner"}>
      {wondowWidth > 768 ? (
        <div>
          <div className={`${"cartProductHeadings"} row col-12`}>
            <div className="col-5 ">
              <span className={"carttableheadings"}>{`Your Item`}</span>
            </div>
            <div className="col-2 text-center">
              <span className={"carttableheadings"}>{`Price`}</span>
            </div>
            <div className="col-2 text-center">
              <span className={"carttableheadings"}>{`Quantity`}</span>
            </div>
            <div className="col-2 text-center">
              <span className={"carttableheadings"}>{`Subtotal`}</span>
            </div>
            <div className="col-1 text-end">
              <span className={"carttableheadings"}></span>
            </div>
          </div>
          <div className={"Add_product_cart_left_container_item"}>
            {AfterDiscount?.map((ele, index) => {
              let wrigh = Boolean(ele.Price.Weight)
                ? ele.Price.Weight
                : `${ele.Price.Unit} Unit`;
              return (
                <div
                  className="row py-3 px-0 border-top border-bottom justify-content-center align-items-center"
                  key={index}
                >
                  <div className="row align-items-center col-5">
                    <div className={"Add_prod_item_image_cont"}>
                      <Link
                        href={`/products/${modifystr(ele.category)}/${modifystr(
                          ele.SubcategoryName
                        )}/${modifystr(ele.ProductName)}/${ele.Product_id}`}
                      >
                        <Image
                          width={100}
                          height={100}
                    ...
                    <div className={"Add_prod_content_cont"}>
                      <h5
                        className={`${"add_prod_cart_p_title"} ${
                          ele?.Price?.Stock !== "IN Stock" && "text-danger"
                        }`}
                        onClick={() => {
                          navigate(ele);
                        }}
                      >
                        {ele.ProductName + "(" + wrigh + ")"}
                      </h5>
                      <h4
                        className={`${"add_prod_cart_p"} ${
                          ele?.Price?.Stock !== "IN Stock" && "text-danger"
                        }`}
                      >
                        {ele.StoreName}
                      </h4>
                      {ele?.Price?.Stock !== "IN Stock" && (
                        <h3
                          className={`add_prod_cart_p text-danger`}
                        >{`This Product is Out of Stock Now Please remove it from the cart`}</h3>
                      )}
                    </div>
                  </div>
                  <div className="col-2 text-center">
                    <span className={"add_prod_span_amount"}>
                      ${parseInt(ele.Price.SalePrice)}
                    </span>
                  </div>
                  <div className="col-2 text-center">
                    <div className={"AddToCartReviewBtn"}>
                      <div className={"addToCart_btn"}>
                        {(Loadingmines || ele.Cart_Quantity) > 1 && (
                          <LoadingButton
                            loading={Loadingmines}
                            style={{ width: "15px" }}
                            onClick={() => {
                              decreaseQuantity(ele.id, ele);
                            }}
                          >
                            {" "}
                            <GrFormSubtract color="gray" />
                          </LoadingButton>
                        )}
                      </div>
                      <div
                        className={"AddToCartCount"}
                        style={{ width: "20px" }}
                      >
                        <p className={"addToCartCountNumber"}>
                          {ele.Cart_Quantity}
                        </p>
                      </div>
                      <div className={"addToCart_btn"}>
                        <LoadingButton
                          ...
                          style={{ width: "15px" }}
                          onClick={() => {
                            Quantity(ele.id, ele.Cart_Quantity, ele);
                          }}
                        >
                  ...
                  <div className="col-2 text-center">
                    <span
                      className={"add_prod_span_amount"}
                      value={ele.Price.SalePrice * ele.Cart_Quantity}
                    >
                      ${parseInt(ele.Price.SalePrice * ele.Cart_Quantity)}
                    </span>
                  </div>
                  <div className="col-1 text-center">
                    <span>
                      <LoadingButton
                        ...
                        style={{ width: "15px" }}
                        onClick={() => {
                          DeleteItem(ele.Product_id, ele.id);
                        }}
                      >
                        {" "}
              ...
              <div className={"addtoproduct_card"} key={index}>
                ...
                <div className={"mb_addtoproduct_card_content"}>
                  <div className="d-flex align-items-start">
                    <h5
                      className={`${`add_prod_cart_p_title`} col-10 ${
                        ele?.Price?.Stock !== "IN Stock" && "text-danger"
                      }`}
                      onClick={() => {
                        Navigate.push(ele);
                    ...
                    <span className="col-2 d-flex justify-content-center align-items-center">
                      <button
                        loading={LoadingDelete}
                        className="center border-0 bg-transparent"
                        onClick={() => {
                          DeleteItem(ele.Product_id, ele.id);
                        }}
                      >
