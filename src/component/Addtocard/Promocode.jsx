...
const PromoCode = () => {
  ...
  function handlechnage() {
    if (state?.login) {
      ...
      axios
        .post(` https://api.cannabaze.com/UserPanel/PromoCodeCheck/`, data, {
  ...
  function clear() {
    axios
      .get(`https://api.cannabaze.com/UserPanel/RemovePromocode/`, {
        headers: { Authorization: `Bearer ${token_data}` },
      })
      .then((res) => {
        dispatch({ type: "ApiProduct", ApiProduct: !state.ApiProduct });
  ...
  return (
    <div className={newclasess.promocode_container}>
      <label
        htmlFor="promocode"
        className={newclasess.promoCode_label}
      >{`Enter a promo code`}</label>
