...
function Notification({
          ...
          res.data.forEach((item) => {
            ...
            if (item.blog.length !== 0) {
              datax.push({
                ...
                link: `/${
                  item.blog[0].category_name === "BLOGS"
                    ? "blogs"
                    : "cannabis-news"
                }/${modifystr(item.blog[0].Title)}/${item.blog[0].id}`,
                Id: item.Notification,
              });
            }
          });
      ...
      response.data.forEach((item) => {
        ...
        if (item.blog.length !== 0) {
          datax.push({
            ...
            link: `/cannabis-news/${modifystr(item.blog[0].Title)}/${
              item.blog[0].id
            }`,
            Id: item.Notification,
          });
  ...
  return (
    notify && (
      ...
      >
        <div
          className={`${clases.notificationList} ${
            !Boolean(notificationdata?.length) && "flex-column center"
          } `}
        >
          <div className={clases.notificationHeader}>
            ...
            {Boolean(
              ...
              state?.login && (
                <span className={clases.clearNotify} onClick={() => ClearAll()}>
                  {" "}
                  <RxCross2 />{" "}
                </span>
              )}
            ...
            <div className={clases.notificationContainer}>
              {Boolean(
                ...
                notificationdata?.map((data, index) => {
                  if (
                    Boolean(
                      !state.Profile.RemovedNotification.includes(data.Id)
                    )
                  ) {
                    return (
                        ...
                        <div className="w-100">
                          <div className="d-flex justify-content-between align-items-center">
                            ...
                            <span
                              ...
                              onClick={(e) => Clear(data)}
                            >
                              <RxCross2 size={15} color={"#000"} />
                            </span>
                          </div>
                          ...
                          >
                            <div className="d-flex align-items-center justify-content-between gap-5">
                              <span className={clases.notify_date}>
                                {calculateTImefromDate(data.date)}
                              </span>
                            </div>
                          </Link>
                        </div>
                    ...
                    <div key={index} className={clases.notification_box}>
                        ...
                        <div className={clases.notification_img}>
                          <div className={clases.notiimgCircle}>
                            <Image
                              ...
                              alt={data.title}
                              title={data.title}
                              onError={() => "/image/weedx.io logo.png"}
                            />
                          </div>
                        </div>
                      </Link>
                      <div className="w-100">
                        ...
                        >
                          {" "}
                          <div className="d-flex align-items-center justify-content-between gap-5">
                            <h4 className={clases.notititle}>{data.title} </h4>{" "}
                          </div>{" "}
                        </Link>
                      </div>
                    </div>
