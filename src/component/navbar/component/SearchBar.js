...
const SearchBar = ({ path }) => {
  ...
  React.useEffect(() => {
    ...
    setLoading(true);
    setOpen(true);
    Axios.post("http://127.0.0.1:1331/UserPanel/Get-HomePageFilter/", {
      ...
      .then((response) => {
        if (response.status === 200) {
          setLoading(false);
          setOpen(true);
          const searchResults = Object.entries(response.data).flatMap(
            ([type, items]) =>
              items.map((data) => ({
                type,
  ...
  const handleSearchAPI = (id, type, t) => {
    if (type === "Product") {
      ...
      router.push(
        `/products/${modifystr(t.Category)}/${modifystr(
          t.SubcategoryName
        )}/${modifystr(t.value)}/${t.id}`
      );
    } else if (type === "Store") {
      setSelctionoption(t.value);
      const route =
        t.Store_Type === "delivery" ? "weed-deliveries" : "weed-dispensaries";
      ...
      setSelctionoption(t.value);
      router.push(
        `/Products/${modifystr(t.Category)}/${modifystr(t.value)}/${t.id}`
      );
  ...
  return (
    <div className="col_Search">
      <div
        ...
        style={{
          display: openLocation && searchBarWidth ? "block" : "inline-flex",
        }}
      >
        <Autocomplete
          ...
          onChange={(event, value) =>
            handleSearchAPI(value?.id, value?.type, value)
          }
            ...
            return (
              <div {...props} key={t.id} className="text-dark">
                <ul className="PopperLIst">
                  <li
                    onClick={() => handleSearchAPI(t.id, t.type, t)}
                    className="searchBarListStyles"
                  >
                    <Image
                      unoptimized
                      src={t.image}
          ...
          renderInput={(params) => (
            <TextField
              ...
              style={{
                ...
                top: "0px",
                display:
                  openLocation && searchBarWidth ? "none" : "inline-flex",
                width: "100%",
              }}
              InputProps={{
                ...params.InputProps,
                ...
                endAdornment: loading && (
                  <CircularProgress style={{ color: "#31B655" }} size={20} />
                ),
              }}
            />
          )}
