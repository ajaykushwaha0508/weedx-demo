  ...
  React.useEffect(() => {
    if (searchval?.length !== 0) {
      const getData = setTimeout(() => {
        Axios.post(
          `https://api.cannabaze.com/UserPanel/SearchProductbyBrand/`,
          {
            brand: id,
            search: searchval,
          }
      ...
      Axios.get(
        `https://api.cannabaze.com/UserPanel/Get-ProductbyBrand/${id}`
      ).then((response) => {
        SetBrandProduct(response.data);
      });
      Axios.get(`https://api.cannabaze.com/UserPanel/Get-BrandById/${id}`).then(
        (response) => {
        ...
        <div className="row  center mx-0 mt-4 mb-4">
          ...
          <div className="col-md-9">
            <div>
              {" "}
              <SearchBar
                style={{ background: "#FFFFF", border: "1px solid #31B665" }}
                value={searchval}
                onChange={(e) => {
                  Setsearchval(e);
                }}
