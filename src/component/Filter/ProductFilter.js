...
const ProductFilter = ({ ProductFilterData, arr, Setarr1, Store_id, id }) => {
  ...
  function Category_Drop(i, name, values = {}) {
    if (name === "Category") {
      ...
      if (
        ref.current.childNodes[0].childNodes[0].childNodes[0].childNodes[1]
          .checked
      ) {
        Axios.post("http://127.0.0.1:1331/UserPanel/UnitFilter/", {
          store: Store_id,
          unit: true,
        }).then((res) => {
          Setarr1(res?.data);
  ...
  return (
    <>
        ...
        <div className="col-12 col-md-2">
          <div className="form-outline" data-mdb-input-init>
            <input
              ...
              className={
                searchtext.length !== 0
                  ? `form-control ${"customSearchBar"}`
                  : `form-control ${"customSearchBar"} ${"customSearchBarsearchicon"}`
              }
            />
          </div>
        </div>
      ...
      <div
        className={` col-lg-3 col-xxl-2 col-md-12 gap-sm-0 gap-2 ${"prod_cat_left_sec"}`}
      >
        {ProductFilterData?.map((ele, index) => {
          ...
          return (
            <div key={index} className={"filter_manu_items"}>
              <div
                className="col-12 d-flex align-items-center px-2 gap-2"
                      ...
                      {Filter.length !== 0 ? (
                        Filter?.map((data, index) => {
                          return (
                            <div key={index}>
                              <div
                                className={`col-10 ${"product_category_dropdown_cursor"}`}
                              >
                                {ele.Name === "Category" ? (
                                  <p
                                    className="m-0"
                                ...
                                return (
                                  SubCategory.CatgoryId === data.id && (
                                    <div
                                      ...
                                      className={`col-10 ${"product_sub_category_dropDown_cursor"}`}
                                    >
                                      <input
                                        type="radio"
                                      ...
                                      />{" "}
                                      <label
                                        htmlFor={SubCategory.SubCategory_name}
                                      >
                                        {SubCategory.SubCategory_name}
                                      </label>
                                    </div>
          ...
          {SortedArrayData.map((ele, index) => {
            ...
            return (
              <div key={index} onClick={() => HandleOpenSortedData(Id, name)}>
                <ol
                  className={"productFilter_sortedList prodfilterSortedListGap"}
                >
