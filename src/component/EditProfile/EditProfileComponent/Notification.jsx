...
const Notification = ({ Profile, Api, SetApi }) => {
    ...
    switch (true) {
      case val === "News_Letter":
        if (Checked.News_Letter === false) {
          ...
          if (
            Checked.News_Letter === true &&
            Checked.Review_Suggestions === false
          ) {
      ...
      case val === "Review_Suggestions":
        if (Checked.Review_Suggestions === false) {
          ...
          if (
            Checked.News_Letter === false &&
            Checked.Review_Suggestions === true
          ) {
  ...
  const handleChange2 = (val) => {
    SetChecked({
      ...Checked,
      Push_Notification: !Checked.Push_Notification,
  ...
  const handleChange3 = (val) => {
    if (val === "Savings" && Checked.Savings === true) {
      SetChecked({
        ...Checked,
        Savings: false,
        Order_Updates_Push_Notifications: false,
        Order_Updates_SMS_Notifications: false,
      });
    } else {
      SetChecked({
        ...Checked,
        Savings: true,
        Order_Updates_Push_Notifications: true,
        Order_Updates_SMS_Notifications: true,
      });
    }
  };
    ...
    switch (true) {
      case val === "Order_Updates_Push_Notifications":
        if (Checked.Order_Updates_Push_Notifications === false) {
          ...
          if (
            Checked.Order_Updates_Push_Notifications === true &&
            Checked.Order_Updates_Push_Notifications === false
          ) {
            SetChecked({
              ...Checked,
              Savings: false,
      ...
      case val === "Order_Updates_SMS_Notifications":
        if (Checked.Order_Updates_SMS_Notifications === false) {
          ...
          if (
            Checked.Order_Updates_Push_Notifications === false &&
            Checked.Order_Updates_SMS_Notifications === true
          ) {
            SetChecked({
              ...Checked,
              Savings: false,
  ...
  React.useEffect(() => {
    Axios.post(
      `https://api.cannabaze.com/UserPanel/Update-UpdateUserProfile/`,
      {
        ...
        ReviewSuggestions: Checked.Review_Suggestions
          ? Checked.Review_Suggestions
          : false,
        PushNotification: Checked.Push_Notification
        ...
        OrderupdatePushNotification: Checked.Order_Updates_Push_Notifications
          ? Checked.Order_Updates_Push_Notifications
          : false,
        OrderupdateSMSNotifications: Checked.Order_Updates_SMS_Notifications
          ? Checked.Order_Updates_SMS_Notifications
          : false,
      },
      {
        headers: {
    ...
    if (Boolean(token_data)) {
      ...
      Axios.get(
        ...
        .then((res) => {
          SetChecked({
            ...Checked,
            Email: res.data.EmailBoolean,
            News_Letter: res.data.NewsLetter,
            Review_Suggestions: res.data.ReviewSuggestions,
            Review_Suggestions: res.data.ReviewSuggestions,
            Push_Notification: res.data.PushNotification,
            Recommendations: res.data.Recommendations,
            Savings: res.data.Savings,
            Order_Updates_Push_Notifications:
              res.data.OrderupdatePushNotification,
            Order_Updates_SMS_Notifications:
              res.data.OrderupdateSMSNotifications,
          });
          // SetProfile(res.data)
        })
        .catch((error) => {
  ...
  const EmailNotification = [
    {
      heading: "News Letter",
      subHeading:
        "Weekly newsletter on the hottest topics and trends in the cannabis community.",
      Value: "News_Letter",
    },
    {
      Value: "Review_Suggestions",
      heading: "Review Suggestions",
      subHeading:
        "Reminders for you to share your experiences about the products and businesses you found on backaroma",
    },
  ];
  const OrderUpdates = [
    {
      Value: "Order_Updates_Push_Notifications",
      heading: "Order Updates - Push Notifications",
      subHeading: "Receive timely updates to track the status of your order",
    },
    {
      Value: "Order_Updates_SMS_Notifications",
      heading: "Order Updates - SMS Notifications",
      subHeading: "Receive timely updates to track the status of your order",
    },
  ];
  return (
              ...
              <div className="row" key={index}>
                <div className="col-10 center justify-content-start">
                  <section className="notification_content_section">
                    ...
                    <p className="editProfileLoginParaStyle ellips">
                      {items.subHeading}
                    </p>
                  </section>
                </div>
                <div className="col-2 notification_col_ReactSwitch">
                  <ReactSwitch
                    ...
                    checkedIcon={false}
                    checked={
                      Checked?.Email === true ? Checked[items.Value] : false
                    }
                    onChange={() => {
                      handleChange1(items.Value);
                    }}
                  />
                </div>
              </div>
              ...
              <div className="row" key={index}>
                ...
                <div className="col-2 notification_col_ReactSwitch">
                  <ReactSwitch
                    ...
                    checkedIcon={false}
                    checked={
                      Checked?.Savings === true ? Checked[items.Value] : false
                    }
                    onChange={() => {
                      handleChange4(items.Value);
                    }}
                  />
                </div>
              </div>
