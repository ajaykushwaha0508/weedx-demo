...
const DeliveryItemsCard = dynamic(
  () =>
    import(
      "../../../component/DeliveriesComponent/DeliveryMenuBar/DeliveryItemsCards"
    ),
  { ssr: true }
);
...
export default function Deliveries(props) {
  ...
  function breadcrumCountry(country, state1, city) {
    if (Boolean(city)) {
      ...
      navigate.replace(
        `/weed-deliveries/in/${modifystr(
          state.Country.toLowerCase()
        )}/${modifystr(state.State.toLowerCase())}/${modifystr(
      ...
      dispatch({ type: "route", route: "" });
      navigate.replace(
        `/weed-deliveries/in/${modifystr(state.Country)}/${modifystr(
          state?.State
        )}`
      );
      ...
      navigate.replace(
        `/weed-deliveries/in/${modifystr(state.Country.toLowerCase())}/`
      );
    }
  }
  return (
        ...
        <div className="row ">
          <div className="headerBoxdescription">
            <h1 className="m-0">
              ...
              <span className="dispensories_name">{locations}</span>
            </h1>
            ...
            {Boolean(props?.store.length) ? (
              <Box className={``} sx={{ width: "100%", typography: "body1" }}>
                <TabContext value={value}>
                  <Box
                    className={`${classes.open_dispensory_tab_background} ${classes.open_dispensory_tab}`}
                  ...
                  <Box className={`${classes.deliverItemCardPadding}`}>
                    <TabPanel value="1">
                      <DeliveryItemsCard Deliverie={props?.store} />
                    </TabPanel>
                    <TabPanel value="2">
                      <DeliveryItemsCard Deliverie={props?.store} />
                    </TabPanel>
              ...
              <Wronglocation
                ...
                description={`Delivery service isn't available at your location. Would you like to try a different address ?`}
              />
            )}
          </div>
          ...
          {contentdata.length !== 0 && contentdata?.Faq[0]?.title !== "" && (
            <>
              {" "}
              ...
              <div className="row">
                {contentdata?.Faq?.map((item, index) => {
                  return (
                    <div className="col-lg-6 webContent my-2" key={index}>
                      <Accordion>
                        <AccordionSummary
...
export const getServerSideProps = async (context) => {
  ...
  if (isDirectHit || isFromGoogle) {
    ...
    const createurl = Boolean(route)
      ? `http://www.weedx.io/weed-deliveries/in/${modifystr(
          country1
      ...
      : Boolean(country1) &&
        `http://www.weedx.io/weed-deliveries/in/${modifystr(country1)}`;
