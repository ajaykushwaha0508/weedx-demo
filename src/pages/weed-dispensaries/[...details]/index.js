...
const NewFlavourBanner = dynamic(
  () => import("../../../component/StoreDetails/NewFlavourBanner"),
  { ssr: true }
);
const StoreDetailMenuItem = dynamic(
  () =>
    import(
      "../../../component/StoreDetails/StoreDetailComponent/StoreDetailMenuItem"
    ),
  { ssr: true }
);
import CategoryProduct from "../../../component/category/category";
const ComponentStoreDetails = dynamic(
  () =>
    import(
      "../../../component/StoreDetails/StoreDetailComponent/ComponentStoreDetails"
    ),
  { ssr: true }
);
...
const StoreDetails = dynamic(
  () => import("../../../component/ScoPage/StoreDetails"),
  { ssr: true }
);
import {
  Store_Add_Review,
  Store_OverAllGet_Review,
  Store_Get_UserComment,
  Store_Get_Review,
  Delete_StoreReview,
  StoreHelpFull,
} from "../../../hooks/apicall/api";
...
export default function DispensoriesDetails(props) {
  ...
  let tab =
    navigate.query.details.length === 2 ? "menu" : navigate.query.details[1];
  ...
  function SelectionTab(item) {
    if (
      Boolean(location.asPath.slice(0, 16) === "/weed-deliveries") ||
      Boolean(location.asPath.slice(0, 18) === "/weed-dispensaries")
    ) {
      navigate.replace(
        item === "Menu"
          ? `${
              location.asPath.slice(0, 16) === "/weed-deliveries"
                ? "/weed-deliveries"
                : "/weed-dispensaries"
            }/${modifystr(Despen[0]?.Store_Name)}/${id}`
          : `${
              location.asPath.slice(0, 16) === "/weed-deliveries"
                ? "/weed-deliveries"
                : "/weed-dispensaries"
            }/${modifystr(Despen[0]?.Store_Name)}/${modifystr(item)}/${id}`,
        0,
        { shallow: true }
      );
    } else {
      navigate.replace(
        `/menu-integration/${modifystr(Despen[0]?.Store_Name)}/${modifystr(
          item
        )}/${id}`,
        0,
        { shallow: true }
  ...
  React.useEffect(() => {
    if (state?.login && state.Profile.id !== undefined && id !== undefined) {
      Store_Get_UserComment(state.Profile.id, id)
        .then((res) => {
  ...
  function navigationtab(route, store, id) {
    if (Boolean(store)) {
      ...
      if (Boolean(state.City)) {
        navigate.push(
          `${route}/in/${state.Country.toLowerCase()}/${state.State.toLowerCase()}/${state.City.toLowerCase()}`
        );
      } else if (Boolean(state.State)) {
        navigate.push(
          `${route}/in/${modifystr(state.Country)}/${modifystr(state.State)}`
        );
      } else {
      ...
      answer: (
        <span
          dangerouslySetInnerHTML={{
            __html: ` ${
              Despen[0].Store_Name
            } is located at  <a target="#" href="http://www.google.com/maps/search/?api=1&query=${encodeURIComponent(
              Despen[0]?.Store_Address
            )}" style="cursor: pointer; color: #31B665; text-decoration: underline;">${
              Despen[0].Store_Address
            }</a>`,
          }}
        />
    ...
    <div>
      ...
      <div className="product_container">
        ...
        <div>
          ...
          {(tab === "menu" || tab === undefined) && (
            <React.Fragment>
              {!productload ? (
                  ...
                  <div className="row">
                    ...
                    <div className={"col-12 col-lg-9 col-xxl-10"}>
                      {Boolean(DespensariesData.length) ? (
                        <ProductList
                          arr={
                            Boolean(categoryProduct.length)
                              ? categoryProduct
                              : DespensariesData.slice(0, 4)
                          }
                          link={
                            Boolean(
                              location.asPath.slice(0, 18) ===
                                "/weed-dispensaries" ||
                                location.asPath.slice(0, 16) ===
                                  "/weed-deliveries"
                            )
                              ? "products"
                              : "menu-integration"
                ...
                Boolean(DespensariesData.length) ? (
                  <div
                    className="col-12 productCat_cont"
          ...
          {tab === "deals" && (
            <div className={newclasess.noReview}>
              <div className={newclasess.noreviewicon}>
                <div className={newclasess.iconcircl}>
                  {" "}
                  <Image
                    ...
                    className={newclasess.nodealsicon}
                    alt="no Deals"
                    title="no Deals"
                  />
                </div>
              ...
              <h3
                className={newclasess.noreview_title}
              >{`Discover More Savings Soon!`}</h3>
              <p className={`${newclasess.noreview_description} w-lg-50`}>
                {`It looks like there are no active deals at the moment at `}
                <Link
                  target="_blank"
                  href={`/weed-dispensaries/${modifystr(
                    Despen[0]?.Store_Name
                  )}/${Despen[0]?.id}`}
                >
                  <b>{Despen[0]?.Store_Name}</b>
                </Link>
                {`. Don't worry, though – our partnered stores frequently update their promotions. Be sure to check back regularly for exciting discounts and special offers on your favorite products.`}
              </p>
              <p className={`${newclasess.noreview_description} w-lg-50`}>
                {`In the meantime, explore the diverse range of products available at `}
                <Link
                  target="_blank"
                  href={`/weed-dispensaries/${modifystr(
                    Despen[0]?.Store_Name
                ...
                {`. We're constantly working to bring you the best deals, so stay tuned for upcoming promotions.`}
              </p>
            </div>
          )}
        </div>
...
const fetchDispensariesAndProducts = async (country, state, city) => {
  ...
  try {
    const [dispensariesResponse, productsResponse] = await Promise.all([
      axios.post("http://127.0.0.1:1331/UserPanel/Get-Dispensaries/", object2, {
...
export async function getStaticProps(context) {
  const storeId = _.findIndex(
    context.params.details,
    (item) => !isNaN(item) && !isNaN(parseFloat(item))
  );
  let data = [];
  let productdata = [];
  let review = [];
  ...
  try {
    ...
    const reviewResponse = await axios.get(
      `http://127.0.0.1:1331/UserPanel/Get-StoreReview/${context.params.details[storeId]}`
    );
    ...
    extradata = await fetchDispensariesAndProducts(
      storeResponse.data[0].Country,
      storeResponse.data[0].State,
      storeResponse.data[0].City
    );
  } catch (error) {
    console.error("Error fetching data:", error);
    return {
  ...
  const isValidStore =
    ...
    (storeId !== 1 &&
      modifystr(data[0].Store_Name) === context.params.details[0] &&
      ["menu", "store-details", "review", "deals"].includes(
        context.params.details[1]
      ) &&
      parseInt(context.params.details[storeId]) === data[0].id);
    ...
    return {
      props: {
        params: {
          ...
          tab: !isNaN(parseInt(context.params.details[1]))
            ? "menu"
            : context.params.details[1],
          storeData: data[0],
          product: productdata,
