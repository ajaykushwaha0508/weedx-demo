...
import WeedDispansires from "../../../component/WeedDispansires/Weed_Dispansires";
const DispensariesSco = dynamic(
  () => import("../../../component/ScoPage/dispensariessco"),
  { ssr: true }
);
...
export default function Dispensaries(props) {
  ...
  const DispensorShopLocation = [
    {
      name: "Weed Dispensaries in",
      city: props.formatted_address || state.Location,
    },
  ];
  ...
  React.useEffect(() => {
    if (props.isDirectHit || props.isFromGoogle) {
      ...
      if (
        props.locationApi === false &&
        props.setCookies === "notnaivigation"
      ) {
        dispatch({ type: "Location", Location: props?.formatted_address });
        ...
        const countryCode =
          countries.getAlpha2Code(props?.location?.country, "en") || "US";
        ...
        if (route) {
          url += `${modifystr(country)}/${modifystr(state)}/${modifystr(
            city
          )}/${modifystr(route)}`;
        } else if (city) {
  ...
  return (
    <div className="w-100 mx-auto  dispensaries_centers">
      <DispensariesSco
        ...
        format_Address={props?.formatted_address}
      ></DispensariesSco>
      <div className="w-100">
        <div className="headerBoxdescription">
          <div style={{ cursor: "pointer" }}>
            <span onClick={() => navigate.push("/")}>{"Home"}</span>
            {Boolean(props.location.country) && (
              <span>
                {" "}
                {">"}{" "}
                <span onClick={() => breadcrumCountry("Country")}>
            ...
            {Boolean(props.location.state) && (
              <span>
                {" "}
                {">"}{" "}
                <span onClick={() => breadcrumCountry("Country", "state")}>
                  {props.location.state}
                </span>
              </span>
            )}
            {Boolean(props.location.city) && (
              <span>
                {" "}
                {">"}{" "}
                <span
                  onClick={() => {
                    Boolean(props.location.route) &&
                      breadcrumCountry("Country", "state", "City");
                  }}
                >
                  {props.location.city}
                </span>
            ...
            {Boolean(props.location.route) && (
              <span>
                {" "}
                {">"} <span>{props.location.route}</span>
              </span>
            )}
          </div>
          {DispensorShopLocation?.map((ele, index) => {
            return (
              <div key={index}>
                <h1 className="m-0">
                  {" "}
                  <span className="dispensories_name">{ele.name}</span>{" "}
                  <span className="dispensories_name">{ele.city}</span>
                </h1>
              </div>
            );
          })}
      ...
      <div className="w-100 dispensory_menu my-2">
        {props.store?.length !== 0 ? (
          <Box
            className={` ${classes.dispensory_tab_background}`}
            sx={{ width: "100%" }}
          >
            <Box
              className={classes.open_dispensory_tab}
            ...
            >
              <TabPanel value={value} index={0}>
                <WeedDispansires
                  ...
                  urlcscr={props.location}
                />
              </TabPanel>
              <TabPanel value={value} index={1}>
                <WeedDispansires
                  Store={props.store.filter((item) => {
          ...
          <Wronglocation
            title={" No dispensaries available"}
            description={
              "We apologize, but it appears that there are no dispensaries available in your location. Would you like to enter a different address to search for a nearby dispensary?"
            }
          />
        )}
      </div>
...
export const getServerSideProps = async (context) => {
  ...
  if (isDirectHit || isFromGoogle) {
    ...
    const decodedLocation = locationParams
      .map((param) => decodeURIComponent(param))
      .reverse()
      .join(" ");

    const k = await Location(
      decodedLocation,
      type,
      context,
      14,
    ...
    (locationApi = k.api), (setCookies = k.cookies);
  } else {
    const cookies = context.req?.headers?.cookie
      ? cookie.parse(context.req.headers.cookie)
      : {};
    cookies.fetchlocation = cookies.fetchlocation
      ? JSON.parse(cookies.fetchlocation)
      : null;
    ...
    const createurl = Boolean(route)
      ? `http://www.weedx.io/weed-dispensaries/in/${modifystr(
          country1
        )}/${modifystr(state)}/${modifystr(city)}/${modifystr(route)}`
      : Boolean(city)
      ? `http://www.weedx.io/weed-dispensaries/in/${modifystr(
          country1
        )}/${modifystr(state)}/${modifystr(city)}`
      : Boolean(state)
      ? `http://www.weedx.io/weed-dispensaries/in/${modifystr(
          country1
        )}/${modifystr(state)}`
      : Boolean(country1) &&
        `http://www.weedx.io/weed-dispensaries/in/${modifystr(country1)}`;
