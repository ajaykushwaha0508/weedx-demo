    ...
    <div className="container">
      <div className="myreviewContainer">
        <div className="w-100 d-flex gap-2 align-items-center my-4">
          <span>
            <IconButton onClick={() => router.push("-1")}>
              <MdOutlineKeyboardArrowLeft color="#000000" />
            </IconButton>
          </span>
          <span onClick={() => router.push(-1)} className="BackPageBtn">
            Back
          </span>
        </div>
        ...
        {!Boolean(allstorereviews.length && allproductreviews.length) ? (
          <div className=" Empty_container_margin_top">
            <div className="EmtyCard_container">
              ...
              <p className="height_empty_div_paragraph ellipsis">{`Don't wait to bake. Add items to your cart and enjoy`}</p>
              <br />
            ...
            {allproductreviews?.map((item) => (
              <div className="myreviewBox" key={item?.review?.id}>
                ...
                <div className="d-flex gap-1">
                  ...
                  {Array.from({ length: 5 - (item?.review?.rating || 0) }).map(
                    (_, index) => (
                      <BsStar
                        size={16}
                        color="#31B665"
                    ...
                    <div className="container-fluid mx-2 review_reply">
                      <div className="d-flex gap-2">
                        ...
                        <div className="related_review_content">
                          <h3 className="reviews_title">
                            Response from the Owner
                          </h3>
                          <p className="reviews_writer">
                            {item?.review?.Store_Name}
                          </p>
                          <div className="review_date">
                            <p>
                              {calculateTimeFromDate(item?.review?.ReplyTime)}
                            </p>
                          </div>
                        </div>
                      </div>
                      <div className="review_description_container">
                        <p>
                          {textGive(item.Reply)}{" "}
                          {item?.Reply?.split(" ")?.length >= 100 && (
                            <span
                              className="band_shlebtn"
                              onClick={() => setReadOpen(!readopen)}
                            >
                              ...Read {readopen ? "More" : "Less"}
                            </span>
                          )}
                        </p>
                      </div>
                ...
                <div className="myreview_footer text-end px-3">
                  <Badge
                    badgeContent={item?.review?.count}
                  ...
                  >
                    {item?.review?.helpfull?.includes(state?.Profile?.id) ? (
                      <AiTwotoneLike
                        ...
                        onClick={() => {
                          state?.login
                            ? handleHelpfulProductReview(item?.review)
                            : router.push("/login");
                        }}
                      />
                    ) : (
            ...
            {allstorereviews?.map((item) => (
              <div className="myreviewBox" key={item.id}>
                ...
                <div className="d-flex gap-1 mb-2 mb-md-3">
                  ...
                  {new Array(Math.max(0, 5 - (item?.rating ?? 0)))
                    .fill(null)
                    .map((_, index) => (
                      <BsStar
                        size={16}
                  ...
                  >
                    {item?.helpfull?.includes(state?.Profile?.id) ? (
                      <AiTwotoneLike
                        ...
                        onClick={() => {
                          state?.login
                            ? handleHelpfulStoreReview(item)
                            : router.pathname.includes("/menu-integration")
                            ? setOpen(true)
                            : router.push("/login");
                        }}
                      />
