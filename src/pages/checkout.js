...
export default function CheckOutMainPage() {
  ...
  const [ShowDeliveryInformation, SetShowDeliveryInformation] =
  ...
  async function SubmitData() {
    ...
    Boolean(Details.MedicalMarijuanaNumber) &&
    ...
    formdata.append(
      "Address",
      state.selectDeliveryoptions === "pickup_btn"
        ? state.AllProduct[0]?.StoreAddress
    ...
    formdata.append(
      ...
      asdsd === "Delivery"
        ...
        : state.AllProduct[0]?.Country
    );
    formdata.append(
      "State",
      asdsd === "Delivery" ? state.DeliveryState : state.AllProduct[0]?.State
    );
    formdata.append(
      "City",
      asdsd === "Delivery" ? state.DeliveryCity : state.AllProduct[0]?.City
    );
    Boolean(state.CoupounAmount) &&
      formdata.append(
        "DiscountedAmount",
    ...
    await Axios.post(
      ...
      .then((response) => {
        ...
        navigate.push(
          {
            pathname:
              location.pathname === "/menu-integration/checkout"
                ? "/menu-integration/order-placed"
        ...
        if (data.data.length !== 0) {
          data.data.map((user, key) => {
            SetDetails({
              ...
              MedicalMarijuanaNumber: user.MedicalMarijuanaNumber,
              MobileNo: user.MobileNo,
            });
          });
        }
        ...
        <div className="col-md-8 col-xl-8 col-lg-7 col-sm-12 col-12">
          <div className={newclases.DeliveryOption_container}>
            <DeliveryOption
              Time={Time}
              SetTime={SetTime}
              Hours={state?.AllProduct[0]?.StoreHours}
              DeliveryOptionData={DeliveryOptionData}
              address={InputValues}
              SetShowData={SetShowData}
            />
            <DeliveryInformation
              SetShowDeliveryInformation={SetShowDeliveryInformation}
              image={image}
...
CheckOutMainPage.getLayout = function getLayout(page) {
