...
const RecentPost = dynamic(
  () => import("@/component/BlogComponent/RecentPost"),
  { ssr: true }
);
import { BsFillShareFill } from "react-icons/bs";
const RecentPostComment = dynamic(
  () => import("@/component/BlogComponent/RecentPostComment"),
  { ssr: true }
);
...
export default function Blogs(props) {
  ...
  let currentPosts = CommentCardArrays?.slice(
    indexOfFirstPost,
    indexOfLastPost
  );
      ...
      .then((response) => {
        Get_Comment(id)
          .then((res) => {
            Setcommnet({
              ...Getcommnet,
              CommentCounts: res.data.CommentCounts,
              UserComment: res.data.Comments,
            });
          })
          .catch((error) => {
            console.error(error);
    ...
    <React.Fragment>
      ...
      <div className="row mx-1">
        <div className="col-12 w-100 row align-items-center justify-content-between blog_searchBar_container px-0">
          <section className=" col-2 backButton_section">
            <div className="col-12 backBtnCol_searchBar_height">
              <Link
                href={props.category === "blogs" ? "/blogs" : "/cannabis-news"}
              >
                <span style={{ marginLeft: "-4px", cursor: "pointer" }}>
                  {" "}
                  <IoChevronBack color="#000000" size={20} />
                </span>
              </Link>
              <Link
                href={props.category === "blogs" ? "/blogs" : "/cannabis-news"}
              >
                <span
                  onClick={() => {
                    Location?.pathname?.split("/")[1] === "cannabis-news"
                      ? router.push(
                          Location?.pathname?.split("/")[1] === "cannabis-news"
                            ? "/cannabis-news"
                            : "/blogs"
        ...
        <div className={"blogEditorContainer"}>
          ...
          <div className={"blog_text_container"}>
            <div className={"Linkofblog"}>
              ...
              <div className={`col viewsBlog BlogSocal`}>
                ...
                <span className="blogViewCounts">
                  {News.ViewCount}{" "}
                  <span className="d-md-block d-none">Views</span>
                </span>
              </div>
              <div
                className={`col viewsBlog BlogSocal`}
                onClick={() => {
                  router.push("#comment");
                }}
              >
                ...
                <span className="blogViewCounts">
                  {Getcommnet.CommentCounts}{" "}
                  <span className="d-md-block d-none"> Comment</span>{" "}
                </span>
              </div>
              <div className={`col viewsBlog BlogSocal`}>
                <IconButton
          ...
          {WishList && (
            <WhisList open1={WishList} SetWishList={SetWishList}></WhisList>
          )}
          <RecentPostComment
            scrolltocomment={scrolltocomment}
            id={id}
            GetUserComment={Getcommnet}
            SetUserComment={Setcommnet}
            Get={GetComment}
          />
        </div>
      </div>
...
export async function getServerSideProps(context) {
  try {
    ...
    if (
      modifystr(data[0].category_name) === blogcategoryname &&
      modifystr(data[0].Title) === title &&
      parseInt(id) === parseInt(data[0].id)
    )
      return {
        props: {
          data: data,
