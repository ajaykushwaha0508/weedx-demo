...
export default function RelatedVerifyBrand(props) {
  ...
  return (
    <div className="container pt-3">
      ...
      <BrandDetailsSeo
        brandname={props.params.brand[0].name.toLowerCase()}
        location={asPath}
        image={props.params.brand[0].Brand_Logo}
      ></BrandDetailsSeo>
...
export async function getServerSideProps(context) {
  ...
  try {
    ...
    if (
      modifystr(context.params.name) === modifystr(brandresponse[0].name) &&
      parseInt(brandresponse[0].id) === parseInt(context.params.id)
    ) {
      brand = brandresponse;
